...
# Every tip gets a bubble title of its own, written by hand. They are keyed on
# a hash of the tip's text, so a rebuild keeps them as long as the sentence in
# the manual stays the same.
TITLES = pathlib.Path(__file__).with_name("tip-titles.json")
...
def add_titles(tips):
  ...
  for tip in tips:
    key = tip_key(tip["text"])
    if key in titles:
      tip["title"] = titles[key]
    else:
      sys.stderr.write(f"no title for {key}: {tip['text']}\n")
  return tips
...
if __name__ == "__main__":
  ...
  json.dump(add_titles(build(sys.argv[1])), sys.stdout, indent=1, ensure_ascii=False)
  sys.stdout.write(";\n")
